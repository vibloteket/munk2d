...
int main(int argc,char **argv) {
  ...
  check((sleeping||queued||strcmp(mode,"unlocked")==0)&&n>0&&repeats>0);
  double elapsed=0;
  ...
  for(int repeat=0;repeat<repeats;repeat++) {
    ...
    for(int i=0;i<n;i++) check(!cpBodyIsSleeping(bodies[i]));
    for(int i=0;i<n;i++) { cpSpaceRemoveBody(space,bodies[i]); cpBodyFree(bodies[i]); }
    cpSpaceRemoveShape(space,trigger); cpShapeFree(trigger);
    cpSpaceFree(space); free(bodies);
  }
