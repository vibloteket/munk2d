...
## Nearest-point queries: defer unused point/gradient output — initial candidate (2026-09-14)
...
- Initial candidate `610cb0e` used GCC/Clang `flatten` and polygon-helper `noinline`, based on earlier dispatch prototypes. These controls were subsequently removed (see follow-up below). No persistent storage or allocations are added.
...
## Nearest-point follow-up: remove compiler-specific inlining controls (2026-09-14)

- Prefer cleaner code and fewer regressing workloads over the last few percent of peak speed. Remove both `flatten` and `noinline` (including the MSVC spelling); retain ordinary `inline` on the shared polygon kernel. No query logic changes.
- Isolated ablation of `610cb0e` found `flatten` redundant: removing it produced a byte-identical executable, including a check against the original delivered binary/configuration. The earlier dispatch explanation did not establish a need in the final source.
- Removing `noinline` traded some large circle/segment throughput for better mixed/local and polygon cases. Seven paired rounds + A/A on the attribute-free source against `280c011`, 1024 shapes, radii 16 / 64 / infinity: circles **-7.08 / -15.98 / -15.85%**; segments **-11.53 / -17.69 / -20.36%**; boxes **-2.13 / -4.10 / -3.00%**; 16-gons **-1.04 / -0.73 / +0.73%**; mixed **-3.28 / -5.02 / -3.35%**. Negative means faster. At 64 shapes/infinity: circles -22.93%, segments -23.56%, mixed -6.37%.
...
## Segment box-query early rejection (2026-09-15)

- Base `d3f1db6`, after both #54 and #55. The final change is in the existing `cpBBSegmentQuery`, not a new BBTree helper: for non-vertical segments, inspect the y interval before x divisions and reject only when that interval is wholly outside [0,1]. Preserve the original vertical path, division formulas, x-then-y min/max combination, endpoint inclusion and returned fractions. No new inlining hints, API/layout/state changes, or tighter traversal pruning.
- The older bounded BBTree prototype did not transfer cleanly to the radius-aware traversal: on this base, GCC left its helper out of line and several cases regressed badly. A small predicate plus the old box query also regressed. Both were rejected. The selected version modifies only the existing box calculation and adds permanent regression coverage.
- GCC15.2 Release/LTO, i5-12400T, CPU2; seven AB/BA pairs plus A/A for 132 cases, with exact returned-result hashes, counts, alpha sums and ordered candidate traces. Geometry: circles/segments/boxes/16-gons/mixed; short/long/horizontal/vertical/fan/outside-scene rays; first/all APIs; radii0/0.75; primary size1024 plus128/8192 mixed scaling.
- Mixed1024, first/all deltas (negative=faster): short radius0 **-11.45/-9.62%**, radius0.75 **-9.24/-7.43%**; long radius0 -3.79/+0.18%, radius0.75 -1.57/+1.32%; fan radius0 -1.85/+1.10%, radius0.75 -2.94/-0.47%; horizontal radius0 -6.17/-2.32%, radius0.75 -5.66/-1.40%. Vertical controls: +0.42/+0.05% at radius0, -1.58/-1.92% at radius0.75.
- **Costs remain:** very cheap outside-scene misses add roughly2–3ns/query (mixed cases +15..19% relative). At8192 mixed shapes, short first/all improves10.66/9.43%, but long all-hit regresses2.76%, fan first/all1.13/2.65%. At128, short first/all improves7.96/8.80%, but fan all-hit+1.37%. Not a universal speedup.
- Twelve additional warmed post-movement/reindex snapshot cases: short first/all improves11.03/9.03% (radius0),9.11/7.12% (radius0.75); long/fan first improves0.72..2.83%; all-hit long/fan ranges+0.05..+1.07%. Movement, index updates, Python/FFI and whole frames are not included in timing. The dynamic shapes share one body; this is not an independently moving-actor simulation.
- Permanent legacy-reference test:1million box/segment inputs with12 cutoffs (12million comparisons), bit-identical complete fractions including misses, extreme/raw floating values, NaN/Inf, unordered boxes, signed zero, parallel/zero-length segments and adjacent cutoffs. Release/strict ASan+UBSan12/12; float box and radius tests pass; C++11 oracle passes. All26 smoke/reference summaries exact with deterministic repeats.
- Ordinary MunkBench six paired rounds + A/A: initial Gear/DampedRotary smoke+2.86/+2.54 and Multifixture/SlowExplosion reference+1.53/+1.07 were not stable in a12-pair follow-up (respectively+0.35/+0.51/-0.11/~0%). Follow-up Gear reference+1.38% remains disclosed. No general stepping gain claimed.
- Reproduce using `tools/segment-query.c`; optional `COUNT_QUERIES` validates candidate traces without timing instrumentation, and `MOVED_SNAPSHOT` prepares translated/reindexed snapshots. The versioned MunkBench protocol is unchanged.
